...
class QvalueModel(nn.Module):
    ...
    # Stateless function: encoded(B, C, K, K) -> q_values(B, A)
...
class Insect:
    ...
    def observe(self, observation: Tensor) -> Tensor: ...
    # observation(B, K, K) -> encoded(B, C, K, K)

    def think(self, encoded: Tensor) -> Tensor:
        # encoded(B, C, K, K) -> q_values(B, A)
        return self.Q_online(encoded)

    def policy(self, q_values) -> Tensor: ...
    # q_values(B, A) -> probs(B, A)
