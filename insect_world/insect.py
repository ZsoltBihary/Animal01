...
from insect_world.insect_model import InsectQModel
...
class Insect(Arthropod):
    ...
    def encode(self, observation: Tensor) -> Tensor:
        ...
        """
        B, K, _ = observation.shape
        C, K_model, K_model2 = self.model.state_shape
        assert K == K_model == K_model2, "K (window size) mismatch"

        # Mask: keep only valid values in range [0, C - 1]
        valid = (observation >= 0) & (observation < C)  # shape: (B, K, K)
        mask = valid.unsqueeze(-1)  # shape: (B, K, K, 1)

        # Clamp to valid range to avoid index errors in one_hot
        clamped = observation.clamp(0, C - 1)  # shape: (B, K, K)

        # One-hot encode and mask invalid entries
        one_hot = F.one_hot(clamped, num_classes=C)  # shape: (B, K, K, C)
